    ...
    fun findCardsInCollection(
        ...
        val sql = """
            SELECT
                ...
                c.game_type,
                c.image_url,
                COALESCE(ps.name, m.set_name) AS set_name,
                COALESCE(ps.tcgdex_id, m.set_code) AS set_code,
                p.collector_number,
                COALESCE(p.rarity, m.rarity) AS rarity,
                COALESCE(p.artist, m.illustrator) AS artist_or_illustrator,
                COALESCE(p.price_eur, m.price_eur) AS price
            ...
            LEFT JOIN pokemon_sets ps ON ps.id = p.set_id
